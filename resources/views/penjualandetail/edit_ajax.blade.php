...
@else
    <form action="{{ url('/penjualandetail/' . $detail->detail_id . '/update_ajax') }}" method="POST" id="form-edit-detail">
        @csrf
        @method('PUT')
        <div id="modal-master" class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select name="penjualan_id" id="penjualan_id" class="form-control" required>
                            ...
                            @foreach($penjualans as $penjualan)
                                <option value="{{ $penjualan->penjualan_id }}" {{ $penjualan->penjualan_id == $detail->penjualan_id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        <label>Jumlah</label>
                        <input type="number" name="jumlah" id="jumlah" class="form-control" value="{{ $detail->jumlah }}" min="1" required>
                        <small id="error-jumlah" class="error-text form-text text-danger"></small>
                    </div>
                    <div class="form-group">
                        <label>Harga</label>
                        <input type="number" name="harga" id="harga" class="form-control" value="{{ $detail->harga }}" required>
